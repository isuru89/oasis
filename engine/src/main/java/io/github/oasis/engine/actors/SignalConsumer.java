...
public class SignalConsumer extends OasisBaseActor {
    ...
    private void initializeMe(StartRuleExecutionCommand message) {
        ...
        sinkLog.info("[{}] Initialization from {}", myId, parentId);
    ...
    private boolean processSignal(SignalMessage signalMessage) {
        ...
        sinkLog.info("[{}] {} processing {} of rule {}", logId, sink, signal, rule);
        try {
            List<Signal> newSignalList = sink.consume(signal, rule, context);

            if (nonNull(newSignalList) && !newSignalList.isEmpty()) {
                sinkLog.info("[{}] Publishing #{} new signal(s) generated by {}...", logId, newSignalList.size(), signal.getClass().getSimpleName());
