...
public class ChallengesModule extends ElementModule {
    ...
    @Override
    public AbstractSink createSink(Class<? extends AbstractSink> sinkImpl, EngineContext context) {
    ...
    public AbstractProcessor<? extends AbstractRule, ? extends Signal> createProcessor(AbstractRule rule, RuleExecutionContext ruleExecutionContext) {
        if (rule instanceof ChallengeRule) {
            ...
            return new ChallengeProcessor(ruleExecutionContext.getEngineContext().getDb(),
                    ruleExecutionContext.getEngineContext().getEventStore(),
                    ruleContext);
        }
        return null;
    }
